...
global_mean<-sum(groups_param[1,1:3]*groups_param[2,1:3])/sum(groups_param[1,1:3])
...
# classical F ANOVA
oneway.test(DV ~ IV, data=database, var.equal=TRUE)$statistic

# welch's W ANOVA
oneway.test(DV ~ IV, data=database, var.equal=FALSE)$statistic
